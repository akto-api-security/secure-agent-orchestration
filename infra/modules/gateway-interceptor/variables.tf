...
variable "approval_agent_runtime_arn" {
  description = <<-EOT
    ARN of the Approval Agent's AgentCore Runtime (from the
...
variable "lambda_timeout_seconds" {
  description = <<-EOT
    Bumped from this Lambda's earlier 10s default to comfortably cover an
    ...
    for how long the Gateway waits on the interceptor -- this is a
    defensive choice, not a value derived from a confirmed AWS limit.
  EOT
  type        = number
  default     = 30
}
