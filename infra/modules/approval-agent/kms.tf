...
# approval-agent/grants.py). ECC_NIST_P256 / SIGN_VERIFY is a real, current
# KMS key spec+usage combination (ECDSA/SHA-256 per FIPS 186-4) -- verified
# against the KMS Sign/Verify API this session, not assumed.
#
...
# Phase 4's interceptor Lambda reasoning for the precedent). The Approval
...
resource "aws_kms_key" "grants" {
  description              = "Phase 5: signs/verifies short-lived approval grants for asl-approval-agent-${var.environment}"
