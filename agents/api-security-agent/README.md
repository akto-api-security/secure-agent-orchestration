...
| `gateway_tool.py` | SigV4-signs `tools/list`/`tools/call` requests to the Gateway, scoped to the `mac-akto-api-mcp` target. Discovers every tool the Gateway lists for this target (not just one hardcoded search tool) and exposes each as a Strands `MCPAgentTool` -- currently `searchDocumentation`, `getPage`, and `sendFeedback`. Cross-target tools are excluded by the `MCP_TARGET_PREFIX` namespace filter. |
