...
@tool
def search_akto_api_security_docs(query: str) -> str:
    """Search Akto's API Security / DAST documentation for a topic and return matching results.

    Use this for any question about API security testing, DAST (dynamic
    application security testing), or how Akto's API security product works.

    Args:
        query: The search query, e.g. a question or topic to look up in the docs.
    """
    logger.info("search_akto_api_security_docs called: query=%r", query)
    try:
    ...
    logger.info("search_akto_api_security_docs returning %d chars via tool=%s", len(text), tool_name)
    return text
